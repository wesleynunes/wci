...
(function($, undefined) {
     ...
     *   });
     */
    var IntraGrid = Widget.extend(/** @lends kendo.ui.Grid.prototype */ {
        /**
         * @constructs
        ...
        _tmpl: function(rowTemplate, alt) {
			...
			{
                //if (groups > 0) {
					var tempRow = String(rowTemplate);
					var temprow = "";
					var substrRow = tempRow.split("var o");
	
					substrRow[1] = "var o"+substrRow[1];
					
					var substr = String(substrRow[1]).split("return o;");
					
					var subSrc = String(substr[0]+" return o;");
					
					
					var substrSrc = subSrc.split("<tr>");

					temprow = substrSrc[0] + "<tr>"; 
	
	                if (groups > 0) {
	                    temprow += groupCells(groups);
	                }
	                
	                temprow += substrSrc[1];
	                
	                rowTemplate = new Function("data", temprow);
	                //rowTemplate = (rowTemplate);
	                //alert("Type: "+ typeof(rowTemplate));
	                //alert("Saida: "+ rowTemplate);
	                //rowTemplate = outRow;
	            //}
			}
        ...
        _rowsHtml: function(data) {
            ...
            for (idx = 0, length = data.length; idx < length; idx++) {
                ...
                calcRow = calcNode(data[idx],calcRow)
                
				if(idx == length-1){
					htmlCalc = rowTemplate(calcRow);
				}
                that._data.push(data[idx]);
            }
			if(that.options.showCalc){
            	html += htmlCalc;
           }
		...
		if(typeof(arr) == 'object') { //Array/Hashes/Objects
			...
			for(var item in arr) {
				...
				if(typeof(value) == 'object') { //If it is an array,
					...
					if($.isNumeric(value)){
						if(dataArr[item]){
							nodArray[item] = (Number(dataArr[item]) + Number(value));
						}else{
							nodArray[item] = (Number(value));
							//alert(" "+item+": "+ dataArr[item]);
						}
						//nodArray[item] = (Number(dataArr[item]) + Number(value));
					}else if(!dataArr[item]){
						nodArray[item] = "-";
					}else{
						nodArray[item] = "-";
					}
					...
					//dumped_text += level_padding + "'" + item + "' => \"" + value + "\"\n";
					//nodArray[nodeNumber] = dataArr[item] value;
				}
				nodeNumber++;
			}
		} else { //Stings/Chars/Numbers etc.
